...
for k in [5, 7, 10]:
    ...
    print(f' {k} '.center(50, '='))
    start = time.time()
    # n_exps_kfold_dmin(data, classes, layers=[n_features, 4, n_classes], lr=0.005, epochs=1000, n_splits=k, n_experiments=2)
    # n_exps_kfold_dmin(data, classes, layers=[n_features, 4, n_classes], lr=0.05, epochs=1000, n_splits=k, n_experiments=2)
    end = time.time()
...
for k in [5, 7, 10]:
    ...
    print(f' {k} '.center(50, '='))
    start = time.time()
    # n_exps_kfold_dmin(data_pca, classes, layers=[len(data_pca[1]), 4, n_classes], lr=0.05, epochs=1000, n_splits=k, n_experiments=2)
    n_exps_kfold_dmin(data_pca, classes, layers=[len(data_pca[1]), 4, n_classes], lr=0.005, epochs=1000, n_splits=k, n_experiments=2)
    end = time.time()
    print(f'Time = {end-start} s') # Segundos y microsegundos
...
nn.fit(X_train, Y_train, lr=0.05, epochs=1000)
